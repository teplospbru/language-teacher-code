...
export const Admin = () => {
  ...
  const { testsData, isLoading: isSubcollectionsLoading } = useSelector(
    ...
    <div className="admin">
      ...
      <div className="admin__subcollections">
        {isSubcollectionsLoading
          ? 'Loading...'
          : testsData.map(({ title, description, example }) => (
              <AdminSubcollection key={title} title={title} description={description} example={example} />
            ))}
      </div>
