...
export const initialState: AdminInitalState = {
  admin: {
    grammar: {
      testsData: [],
      isLoading: true,
...
export const adminSlice = createSlice({
  ...
  reducers: {
    setSubcollectionsListData(state, action) {
      state.admin.grammar.testsData = action.payload;
      state.admin.grammar.tests = (action.payload as TestsData[]).map((item) => ({
        title: item.title,
        isLoading: true,
        isOpen: false,
      }));
      state.admin.grammar.isLoading = false;
      state.admin.task.tests = (action.payload as TestsData[]).map((item) => ({ title: item.title, questions: [] }));
    },
    setSubcollectionDocs(state, action) {
      state.admin.grammar.tests = state.admin.grammar.tests.map((item) => {
    ...
    setSubcollectionOpen(state, action) {
      const index = state.admin.grammar.tests.findIndex((item) => item.title === action.payload.title);
      state.admin.grammar.tests = state.admin.grammar.tests.map((item, i) => {
        if (i === index) {
